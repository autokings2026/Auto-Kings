...
import { CheckCircle, XCircle, Loader2, AlertTriangle, Car, Wrench } from 'lucide-react'
import { Button } from '@/components/ui/button'


interface CotizacionAdicionalData {
  token: string
  numero: string
...
export function AprobacionAdicionalView({ token }: { token: string }) {
  const [data, setData] = useState<CotizacionAdicionalData | null>(null)
  ...
  useEffect(() => {
    fetch(`/api/cotizacion-adicional/public/${token}`)
  ...
  const responder = async (aprobado: boolean) => {
    ...
    try {
      const res = await fetch(`/api/cotizacion-adicional/public/${token}/responder`, {
        method: 'POST',
      ...
      <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
        <div className="text-center space-y-4 max-w-sm">
          ...
          <h1 className="text-2xl font-bold text-white">
            {aprobado ? '¡Cotización adicional aprobada!' : 'Cotización adicional rechazada'}
          </h1>
          <p className="text-gray-400 text-sm">
            {aprobado
              ? 'Hemos recibido su aprobación. Nuestro técnico continuará con la reparación incluyendo estas piezas.'
              : 'Hemos recibido su respuesta. La reparación continuará sin estas piezas. Si tiene alguna pregunta, contáctenos.'}
          </p>
      ...
      <div className="bg-gray-900 border-b border-gray-800 px-4 py-4">
        <div className="max-w-lg mx-auto flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">Cotización adicional · OT {data.numero}</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto p-4 space-y-5 pb-10">
        ...
        <div className="pt-2">
          ...
          <p className="text-gray-400 text-sm mt-1">
            Mientras reparábamos su {data.vehiculo} (placa {data.placa}) encontramos algo adicional que no
            estaba en la cotización original. Por favor revíselo y apruebe o rechace.
          </p>
        </div>

        {/* Motivo */}
        <div className="bg-amber-950/40 border border-amber-700/30 rounded-xl p-4 flex gap-3">
          <Wrench className="h-4 w-4 text-amber-400 shrink-0 mt-0.5" />
          <div>
            <p className="text-xs text-amber-400 uppercase tracking-wider font-medium mb-1">¿Por qué es necesario?</p>
            <p className="text-sm text-amber-100/90">{data.motivo}</p>
          </div>
        </div>
        ...
        <div className="bg-gray-900 rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-800">
            <p className="text-xs text-gray-500 uppercase tracking-wider font-medium">Trabajos y materiales adicionales</p>
          </div>
          ...
          <div className="border-t border-gray-800 px-4 py-3 space-y-1.5">
            ...
            <div className="flex justify-between text-base font-bold text-white pt-1 border-t border-gray-800">
              <span>Total adicional</span><span className="text-amber-400">{fmt(data.totalGeneral)}</span>
            </div>
          </div>
        </div>
        ...
        {rechazando && (
          <div className="bg-gray-900 rounded-xl p-4 space-y-3">
            ...
            <textarea
              ...
              placeholder="Ej: Prefiero revisarlo en otra ocasión…"
            />
