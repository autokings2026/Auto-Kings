...
export async function generarPdfReporte(id: string): Promise<{ buffer: Buffer; filename: string }> {
  ...
  // Cliente / Vehículo
  const bW = (CW - 12) / 2, bH = 75, bY = doc.y
  ...
  doc.fontSize(8.5).fillColor('#475569') .font('Helvetica')     .text(`Km: ${r.kilometraje.toLocaleString('es-HN')}  |  Técnico: ${r.tecnico}`, rX + 8, bY + 44, { width: bW - 16, lineBreak: false, ellipsis: true })
  doc.y = bY + bH + 16
  ...
  if (r.diagnostico) {
    ...
    const COLS = [
      ...
      { x: M + 200, w: 76,  align: 'left'  as const },
      { x: M + 280, w: 38,  align: 'right' as const },
      { x: M + 322, w: 86,  align: 'right' as const },
      { x: M + 412, w: 83,  align: 'right' as const },
    ] as const
    ...
    ;[
      ...
      doc.rect(totX, ry, totW, 16).fill('#f1f5f9')
  ...
  if (r.encuesta?.respondidoEn) {
    ...
    const eY = doc.y
    doc.fontSize(9).fillColor('#334155').font('Helvetica')
       .text(`Calidad: ${stars(enc.calidad!)} ${enc.calidad}/5`,  M + 6, eY,      { width: CW - 12, lineBreak: false })
       .text(`Tiempo:  ${stars(enc.tiempo!)} ${enc.tiempo}/5`,    M + 6, eY + 14, { width: CW - 12, lineBreak: false })
       .text(`Atención: ${stars(enc.atencion!)} ${enc.atencion}/5`, M + 6, eY + 28, { width: CW - 12, lineBreak: false })
    doc.fontSize(10).fillColor('#0f172a').font('Helvetica-Bold')
       .text(`Promedio: ${+((enc.calidad! + enc.tiempo! + enc.atencion!) / 3).toFixed(1)} / 5`, M + 6, eY + 44, { width: CW - 12, lineBreak: false })
    doc.y = eY + 58
    if (enc.comentario) doc.fontSize(8.5).fillColor('#64748b').font('Helvetica').text(`"${enc.comentario}"`, M + 6, doc.y, { width: CW - 12 })
    doc.moveDown(0.4)
  }
  ...
  for (let i = range.start; i < range.start + range.count; i++) {
    ...
    doc.fontSize(7).fillColor('#94a3b8').font('Helvetica')
       .text(`${tallerNombre}  |  Generado el ${new Date().toLocaleDateString('es-HN')}  |  Pag. ${i - range.start + 1} de ${range.count}`,
